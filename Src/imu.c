...
{
    return HAL_I2C_Mem_Read(&hi2c1, LIS3MDL_ADDR, reg, 1, (uint8_t*) val,
            sizeof(*val), 100);
}
...
{
    return HAL_I2C_Mem_Write(&hi2c1, LSM6DS0_ADDR, reg, 1, (uint8_t*) &val,
            sizeof(val), 100);
}

static int lsm6ds0_ReadReg(uint8_t reg, int8_t *val)
{
    return HAL_I2C_Mem_Read(&hi2c1, LSM6DS0_ADDR, reg, 1, (uint8_t*) val,
            sizeof(*val), 100);
}
